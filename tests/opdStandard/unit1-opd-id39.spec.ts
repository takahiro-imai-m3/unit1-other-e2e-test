...
/**
 * Unit1_OPD_標準テスト_ID39 - 完全版
 *
 * テスト目的:
 * - OPDメッセージ作成機能のテスト
 * - 端末向きがランドスケープでも表示が崩れないことを確認
 *
 * 前提条件:
 ...
 * 1. OPD作成（OPEX管理画面）
 * 2. MR君管理画面でターゲット設定
 * 3. M3 SP版でOPD表示確認（ランドスケープ）
 */
test.describe('Unit1_OPD_標準テスト_ID39', () => {
  let opdId: string;
  let opdTitle: string;

  test('OPD作成 → MR君ターゲット設定 → SP版確認', async ({ browser }) => {
    console.log('#### Unit1_OPD_標準テスト_ID39 - 完全版');
    ...
    // タイトルを生成
    const flowID = process.env.TEST_FLOW_ID || '39';
    ...
    // ========================================
    console.log('\n### Part 3: M3 SP版でOPD表示確認（ランドスケープ）');

    // M3 SP版用のコンテキスト（ランドスケープモード、プロキシ経由）
    const m3spContext = await browser.newContext({
      viewport: { width: 740, height: 360 }, // Galaxy S8 ランドスケープ
    ...
    expect(opdTitle).toContain('自動テストタイトル39');

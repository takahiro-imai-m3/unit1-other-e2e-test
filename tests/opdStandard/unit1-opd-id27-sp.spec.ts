 ...
 */
test.describe('Unit1_OPD_標準テスト_ID27_SP', () => {
  let opdId: string;
  let opdTitle: string;

  test('QFB回答機能の検証 (回答上限なし)', async ({ browser }) => {
    console.log('#### Unit1_OPD_標準テスト_ID27_SP');
    ...
    opdTitle = `自動テストタイトル${flowID}_SP_QFB回答_${opdMessageNumber}_${randomAlnum}`;
    ...
    expect(opdTitle).toContain('自動テストタイトル27_SP_QFB回答');

 ...
 *
 * 注記:
 * - SP版でもOPEX/MR君の操作はPC版と同じ
 * - MA君・ワンポイント医療情報の確認は将来実装予定
 */
test.describe('Unit1_OPD_標準テスト_ID70_SP', () => {
  let opdId: string;
  let opdTitle: string;

  test('OPD作成（ワンポイントMA有効） → MR君ターゲット設定', async ({ browser }) => {
    console.log('#### Unit1_OPD_標準テスト_ID70_SP');
    ...
    expect(opdTitle).toContain('自動テストタイトル70_SP');

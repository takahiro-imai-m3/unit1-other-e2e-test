 ...
 */
test.describe('Unit1_OPD_標準テスト_ID37', () => {
  let opdId: string;
  let opdTitle: string;

  test('QFB回答上限機能の検証 (回答上限: 1)', async ({ browser }) => {
    console.log('#### Unit1_OPD_標準テスト_ID37');
    ...
    opdTitle = `自動テストタイトル${flowID}_QFB上限1_${opdMessageNumber}_${randomAlnum}`;
    ...
    opdId = await opdCreatePage.createOPDMessage({
      ...
      pcDetailBody: 'PCディテール本文コンテンツ（QFB回答上限テスト）',
      useQfb: true,
    ...
    expect(opdTitle).toContain('自動テストタイトル37_QFB上限1');

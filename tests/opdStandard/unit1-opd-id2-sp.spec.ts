...
/**
 * Unit1_OPD_標準テスト_ID2_SP
 *
 * テスト目的:
 * - OPDコピー作成機能の検証（SP版）
 * - 課金対象者となる会員でコピー作成したメッセージがSP版一覧に表示されること
 *
 * 前提条件:
 ...
 * 2. OPDコピー作成
 * 3. MR君管理画面でターゲット設定
 * 4. M3.com SP版でOPD一覧確認（iPhone 15 Plus ランドスケープ）
 * 5. コピーしたOPDの配信停止
 */
test.describe('Unit1_OPD_標準テスト_ID2_SP', () => {
  let opdId: string;
  let copiedOpdId: string;
  let opdTitle: string;

  test('OPD作成 → コピー → MR君ターゲット設定 → M3.com SP版確認 → 配信停止', async ({ browser }) => {
    console.log('#### Unit1_OPD_標準テスト_ID2_SP');
    ...
    opdId = await opdCreatePage.createOPDMessage({
      ...
      pcDetailBody: 'PCディテール本文コンテンツ（SP版テスト）',
    });
    ...
    // ========================================
    // Part 4: M3.com SP版でOPD一覧確認（iPhone 15 Plus ランドスケープ）
    // ========================================
    console.log('\n### Part 4: M3.com SP版でOPD一覧確認（iPhone 15 Plus ランドスケープ）');

    const m3spContext = await browser.newContext({
      viewport: { width: 932, height: 430 }, // iPhone 15 Plus landscape
    ...
    expect(opdTitle).toContain('自動テストタイトル2_SP');

...
/**
 * Unit1_OPD_標準テスト_ID75
 *
 * テスト目的:
 * - S3 CSV自動配信機能のテスト（利用しない）
 * - 小分け配信ファイルのアップロード機能のテスト
 * - 自動配信を利用しない場合、小分け配信されないことの確認
 *
 * 前提条件:
 ...
 *
 * 期待値:
 * - 自動配信の仕様に準拠した挙動をすること（利用しないので配信しないこと）
 * - S3にアップロードしたCSVファイルがあっても小分け配信OPDが作成されないこと
 * - OPD一覧で最初に表示されるOPD IDが変化しないこと
 *
 * このテストは以下を実行します:
 * 1. OPD作成（OPEX管理画面）
 * 2. 自動配信のラベル表示とスイッチ状態を確認（利用しない=OFF）
 * 3. MR君管理画面でターゲット設定
 * 4. QA用ツールで小分け配信ファイルアップロード（systemCd2=901588）
 * 5. OPD標準テスト自動化のjob管理画面で配信実行・確認
 * 6. 小分け配信されていないことを確認（OPD IDが変化しない）
 */
test.describe('Unit1_OPD_標準テスト_ID75', () => {
  let opdId: string;
  let opdTitle: string;

  test('OPD作成 → 自動配信なし確認 → ターゲット設定 → 小分け配信されないことを確認', async ({ browser }) => {
    console.log('#### Unit1_OPD_標準テスト_ID75');
    ...
    opdId = await opdCreatePage.createOPDMessage({
      ...
      managementMemo: 'opd_標準テスト_事前ID75',
    });

    console.log(`✓ OPD作成完了: ID=${opdId}, タイトル=${opdTitle}`);

    // ========================================
    // Part 2: 自動配信のラベル表示とスイッチ状態を確認（利用しない=OFF）
    // ========================================
    console.log('\n### Part 2: 自動配信のラベル表示とスイッチ状態を確認');

    // OPD編集ページに遷移（作成直後の画面で確認）
    const opdEditPage = new OPDEditPage(opexPage);
    await opdEditPage.goto(opdId, proxyNumber);
    await opdEditPage.waitForPageLoad();

    // 自動配信のラベル「利用する」が表示されていることを確認
    await opdCreatePage.verifySubdivideDistributionLabel();

    // 自動配信のスイッチがOFF（利用しない）であることを確認
    await opdCreatePage.verifySubdivideDistributionState(false);

    console.log('✓ 自動配信のデフォルト状態確認完了: OFF（利用しない）');
    ...
    // systemCd2=901588 (ID75用のシステムコード)
    const qaToolUrl = `http://mrqa1/admin/qa/uploadOpdKowakeFile.jsp?systemCd2=901588`;
    ...
    // 取り込み対象となるファイルのプレフィックスに「M3_OPD_ID75_yyyymmdd」を入力
    const opdKowake = `M3_OPD_ID75_${opdMessageNumber}`;
    console.log(`\n📝 プレフィックス入力: ${opdKowake}`);
    ...
    // ========================================
    // Part 6: 小分け配信されていないことを確認（OPD IDが変化しない）
    // ========================================
    console.log('\n### Part 6: 小分け配信されていないことを確認');
    ...
    // MR君管理画面のOPD一覧画面に遷移（管理メモにopd_標準テスト_事前ID75と入力して検索）
    const mrkunSearchUrl = `https://mrkun.m3.com/admin/restricted/mt/OnePointDetail/list.jsp?pointCompanyCd=&productName=&memo=opd_%E6%A8%99%E6%BA%96%E3%83%86%E3%82%B9%E3%83%88_%E4%BA%8B%E5%89%8DID75&opdId=&action=view`;
    ...
    expect(opdTitle).toContain('自動テストタイトル75');

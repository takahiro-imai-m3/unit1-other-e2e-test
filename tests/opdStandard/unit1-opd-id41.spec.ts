...
/**
 * Unit1_OPD_標準テスト_ID41
 *
 * テスト目的:
 * - OPDメッセージ作成機能のテスト
 * - iOS端末のランドスケープ向きで表示が崩れないことを確認
 *
 * 前提条件:
 ...
 * 1. OPD作成（OPEX管理画面）
 * 2. MR君管理画面でターゲット設定
 * 3. M3 SP版でOPD表示確認（iOS ランドスケープ）
 */
test.describe('Unit1_OPD_標準テスト_ID41', () => {
  let opdId: string;
  let opdTitle: string;

  test('OPD作成 → MR君ターゲット設定 → SP版確認(iOS ランドスケープ)', async ({ browser }) => {
    console.log('#### Unit1_OPD_標準テスト_ID41');
    ...
    // タイトルを生成
    const flowID = process.env.TEST_FLOW_ID || '41';
    ...
    // ========================================
    console.log('\n### Part 3: M3 SP版でOPD表示確認（iOS ランドスケープ）');

    // M3 SP版用のコンテキスト（ランドスケープモード、プロキシ経由）
    const m3spContext = await browser.newContext({
      viewport: { width: 812, height: 375 }, // iPhone X ランドスケープ
    ...
    expect(opdTitle).toContain('自動テストタイトル41');

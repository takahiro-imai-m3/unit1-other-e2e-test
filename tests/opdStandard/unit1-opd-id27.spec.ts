...
/**
 * Unit1_OPD_標準テスト_ID27
 *
 * テスト目的:
 * - QFB回答機能の検証
 * - QFB機能が有効化されたOPDが正しく作成されること
 * - ターゲット設定が正常に完了すること
 *
 * 前提条件:
 ...
 */
test.describe('Unit1_OPD_標準テスト_ID27', () => {
  let opdId: string;
  let opdTitle: string;

  test('QFB回答機能の検証 (回答上限なし)', async ({ browser }) => {
    console.log('#### Unit1_OPD_標準テスト_ID27');

    // ========================================
    // Part 1: OPD作成（QFB機能ON、回答上限なし）
    // ========================================
    console.log('\n### Part 1: OPD作成（QFB機能ON、回答上限なし）');
    ...
    opdTitle = `自動テストタイトル${flowID}_QFB回答_${opdMessageNumber}_${randomAlnum}`;

    // OPDメッセージを作成（QFB機能ON、回答上限なし）
    opdId = await opdCreatePage.createOPDMessage({
      ...
      pcDetailBody: 'PCディテール本文コンテンツ（QFB回答機能テスト）',
      useQfb: true,
    ...
    expect(opdTitle).toContain('自動テストタイトル27_QFB回答');

    console.log('\n✅ テスト完了');
    console.log('📝 注記: QFB回答機能（M3.com側での回答操作・通知確認）は将来実装予定');
  });
});

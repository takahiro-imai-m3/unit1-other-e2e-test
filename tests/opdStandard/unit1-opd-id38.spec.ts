...
/**
 * Unit1_OPD_標準テスト_ID38
 *
 * テスト目的:
 * - OPDメッセージ作成機能のテスト
 * - Android端末のポートレート向きで表示が崩れないことを確認
 *
 * 前提条件:
 ...
 * 1. OPD作成（OPEX管理画面）
 * 2. MR君管理画面でターゲット設定
 * 3. M3 SP版でOPD表示確認（Android ポートレート）
 */
test.describe('Unit1_OPD_標準テスト_ID38', () => {
  let opdId: string;
  let opdTitle: string;

  test('OPD作成 → MR君ターゲット設定 → SP版確認(Android ポートレート)', async ({ browser }) => {
    console.log('#### Unit1_OPD_標準テスト_ID38');
    ...
    // タイトルを生成
    const flowID = process.env.TEST_FLOW_ID || '38';
    ...
    // ========================================
    console.log('\n### Part 3: M3 SP版でOPD表示確認（Android ポートレート）');
    ...
    expect(opdTitle).toContain('自動テストタイトル38');
